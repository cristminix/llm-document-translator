 ...
 */
export const runLLM = async (baseUrl, apiKey, modelName, userInput, targetLanguage) => {
    // console.log({
    ...
    const makeRequest = async () => {
        try {
            const promptTemplate = process.env.PROMPT_TEMPLATE || 'Translate the following content to {{targetLanguage}}:\n\n{{userInput}}';
            const prompt = promptTemplate.replace('{{targetLanguage}}', targetLanguage).replace('{{userInput}}', userInput);
            ...
            if (retries < MAX_RETRIES) {
                ...
                throw new Error(`Error running LLM after ${MAX_RETRIES} retries: ${error.message}`);
            }
        }
    };
