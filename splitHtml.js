...
export async function splitHtml(htmlFilePath) {
    ...
    try {
        ...
        const pages = await page.evaluate((PAGE_HEIGHT) => {
            ...
            function splitLargeElement(el, PAGE_HEIGHT) {
                ...
                for (const child of children) {
                    ...
                    if (h > PAGE_HEIGHT) {
                        ...
                        const subpages = splitLargeElement(tempDiv.firstChild, PAGE_HEIGHT);
                ...
                const wrappedPages = pages.map(pageArr => {
            ...
            for (const el of blocks) {
                ...
                if (h > PAGE_HEIGHT) {
                    ...
                    document.body.appendChild(tempDiv);
                    const splitPages = splitLargeElement(tempDiv.firstChild, PAGE_HEIGHT);
                    document.body.removeChild(tempDiv);
                    if (current.length > 0) {
                    ...
                    resultPages.push(...splitPages.map(pg => pg)); // masukkan hasil split per halaman
                    continue;
                }
