...
export const readFileInChunks = async (filePath) => {
...
export const writeOutputFile = async (outputPath, content) => {
    return fs.promises.writeFile(outputPath, content, 'utf8');
};

export const getChunkFilePath = (filePath, chunkIndex) => {
    const fileName = path.basename(filePath);
    return path.join(CHUNK_RESULTS_DIR, `${fileName}.chunk${chunkIndex}.${filePath.split('.').pop() ?? "txt"}`);
};

export const readChunkResult = async (chunkFilePath) => {
    try {
...
export const writeChunkResult = async (chunkFilePath, result) => {
    await ensureChunkResultsDir();
    await fs.promises.writeFile(chunkFilePath, result, 'utf8');
};

export const deleteChunkResults = async (filePath) => {
    const fileName = path.basename(filePath);
